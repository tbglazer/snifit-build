	...
	private URL[] getClasspathURLEntries() {
		...
		if (extRegistry != null) {
			IConfigurationElement[] bundlesCPEntries = extRegistry
					.getConfigurationElementsFor("il.co.fibi.snifit.etools.j2ee.ant", "bundleClasspathEntries");
	...
	private final void displayAllJobsState(boolean displayConsoleMessage) {
		...
		if (allJobs.length > 0) {
			...
			try {
				...
				Status status = new Status(4, "il.co.fibi.snifit..etools.j2ee.ant", 0, ioEx.getMessage(), ioEx);
				...
				if (bufferedWriter != null)
					try {
						...
						Status status = new Status(1, "il.co.fibi.snifit.etools.j2ee.ant", 0, ioEx.getMessage(), ioEx);
						AntCorePlugin.getPlugin().getLog().log(status);
					}
			}
		}
