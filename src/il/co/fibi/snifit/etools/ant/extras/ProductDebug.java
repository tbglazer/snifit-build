...
//          } 
				...
				for (int i = 0; i < allBundles.length; i++) {
					...
					String bundleName = allBundles[i].getHeaders(null).get("Bundle-Name");
					String bundleProvider = allBundles[i].getHeaders(null).get("Bundle-Vendor");
	...
	protected final void validateAttributes(IProgressMonitor monitor) throws BuildException {
		...
		} else if (!this.action.equals(ACTION_LIST_FEATURES) && !this.action.equals(ACTION_PAUSE_APP)
				&& !this.action.equals(ACTION_LIST_PLUGINS)) {
			handleError(NLS.bind(NLSMessageConstants.DEBUG_TASK_ACTION_INVALID,
					new String[] { this.action, ACTION_LIST_FEATURES, ACTION_PAUSE_APP, ACTION_LIST_PLUGINS }));
		}
	}
	...
	private final IBundleGroup[] getFilteredBundleGroups() {
		...
		if (!this.filters.isEmpty()) {
			filteredBundleGroups = new ArrayList<>(400);
			...
			while (bundleGroupIterator.hasNext()) {
				...
				while (filterIterator.hasNext()) {
					String filterName = filterIterator.next().getName();
	...
	private final Bundle[] getFilteredBundles() {
		...
		if (this.filters.isEmpty()) {
			...
			List<Bundle> newBundles = new ArrayList<>(this.filters.size());
			Iterator<ProductDebugFilter> filterIterator = this.filters.iterator();
			while (filterIterator.hasNext()) {
				String bundleName = filterIterator.next().getName();
				Bundle b = Platform.getBundle(bundleName);
				if (b != null) {
					newBundles.add(b);
					continue;
				}
				log(NLS.bind(NLSMessageConstants.DEBUG_TASK_FILTER_NO_BUNDLE_FOUND, bundleName), 0);
			}
