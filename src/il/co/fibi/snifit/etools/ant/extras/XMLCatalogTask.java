...
public class XMLCatalogTask extends FailOnErrorTask {
  public void execute() throws BuildException {
    ...
    try {
      ...
      if (this.catalogLocation != null) {
        ...
        INextCatalog newCatalog = (INextCatalog)defaultCatalog
          .createCatalogElement(10);
        ...
        if (this.verbose)
          log(
              ResourceHandler.getString("XMLCatalog.addNextCatalogSuccess", new Object[] { newCatalog.getCatalogLocation(), defaultCatalog.getLocation() })); 
      } 
      if (this.key != null) {
        ...
        for (int i = 0; i < xmlCatalogs.length; i++) {
          if ("user_catalog".equals(xmlCatalogs[i].getId())) {
            ...
            if (this.verbose)
              log(
                  ResourceHandler.getString("XMLCatalog.addUserCatalogSuccess", new Object[] { userCatalog.getLocation() })); 
            break;
          } 
        } 
      } 
  ...
  private void saveCatalog(ICatalog catalog, IProgressMonitor monitor) throws BuildException {
    if (catalog != null)
      try {
        ...
        handleError(
            ResourceHandler.getString("XMLCatalog.saveFailure", catalog.getLocation()), 
            ioEx);
      }  
  }
  ...
  protected void validateAttributes(IProgressMonitor monitor) throws BuildException {
    if (AntTrace.EXTRAS_TRACE_ENABLED) {
      ...
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
          "Value of attribute \"key\":" + this.key);
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
          "Value of attribute \"uri\":" + this.uri);
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
    ...
    if (this.catalogLocation != null) {
      ...
      if (!f.exists())
        handleError(
            ResourceHandler.getString("XMLCatalog.invalidNextCatalogLocation", this.catalogLocation)); 
    } 
  }
