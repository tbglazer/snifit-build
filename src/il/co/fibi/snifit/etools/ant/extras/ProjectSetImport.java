...
public class ProjectSetImport extends FailOnErrorTask {
        ...
        if (references == null || references.size() == 0) {
          if (this.existingProjects) {
            String msg = ResourceHandler.getString(
                "ProjectSetImport.onlyExistingProjects", this.projectSetFileName);
            log(msg);
            continue;
          } 
          handleError(
              ResourceHandler.getString("Common.noProjectReferences", this.projectSetFileName));
          continue;
        } 
        log(ResourceHandler.getString("ProjectSetImport.providerId", providerId));
        log(
            ResourceHandler.getString("ProjectSetImport.projectsToImport", Integer.toString(references.size())));
  ...
  private IProject[] getIProjectsFromReferences(List<String> references) {
    ...
    for (int i = 0; i < references.size(); i++) {
      ...
      if (parts.length < 4) {
        handleError(
            ResourceHandler.getString("Common.missingProjectNameFromReference", line));
      } else {
  ...
  private List<IProject> teamImportProjects(String providerId, List<String> references, ProjectSetContentHandler handler) {
    ...
    ProjectSetSerializationContext pssc = new ProjectSetSerializationContextForAntTask(
        this.projectSetFileName, overwriteProject);
    ...
    if ((newReferences == null || newReferences.size() == 0) && handler.isVersionOne()) {
      ...
      if (providerType == null) {
        handleError(
            ResourceHandler.getString("ProjectSetImport.missingProviderType", (Object[])new String[] { providerId, this.projectSetFileName }));
      } else {
        ...
        if (serializer != null && newReferences != null)
          try {
            projects = serializer.addToWorkspace(newReferences.<String>toArray(new String[newReferences.size()]), 
                pssc, monitor);
          } catch (TeamException teamEx) {
            ...
            handleError(
                ResourceHandler.getString("Common.exception", ResourceHandler.getString("ProjectSetImport.CVSAuthenticationIncorrect")), isex);
          } catch (Exception ex) {
    ...
    if (projects != null) {
      ...
      for (int i = 0; i < projects.length; i++) {
        log(
            ResourceHandler.getString("ProjectSetImport.projectImported", (Object[])new String[] { Integer.toString(i), projects[i].getName() }));
  ...
  protected void validateAttributes(IProgressMonitor monitor) throws BuildException {
    ...
    if ((this.USERID != null && this.PASSWORD == null) || (this.USERID == null && this.PASSWORD != null))
      handleError(
          ResourceHandler.getString("ProjectSetImport.invalidUserIdPasswordPair")); 
  }
}
