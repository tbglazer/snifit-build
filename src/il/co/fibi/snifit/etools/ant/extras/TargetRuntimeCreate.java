...
import il.co.fibi.snifitws.ast.st.core.internal.WebSphereCorePlugin;
import java.io.File;
...
public class TargetRuntimeCreate extends FailOnErrorTask {
    ...
    try {
      ...
      if (runtimeType == null) {
        handleError(ResourceHandler.getString(
              "TargetRuntimeCreate.missingRuntimeTypeId", this.runtimeTypeId));
      } else {
        if (runtimeNameInUse(this.runtimeName)) {
          if (runtimeAlreadyExists(this.runtimeName, this.runtimeLocation)) {
            log(
                ResourceHandler.getString("TargetRuntimeCreate.runtimeExists", (Object[])new String[] { this.runtimeName, this.runtimeLocation }));
            return;
          } 
          handleError(
              ResourceHandler.getString("TargetRuntimeCreate.invalidName", this.runtimeName));
        } 
        IRuntimeWorkingCopy workingCopy = null;
        try {
          workingCopy = runtimeType.createRuntime(this.targetId, (IProgressMonitor)new NullProgressMonitor());
        } catch (CoreException e) {
          handleError(ResourceHandler.getString(
                "TargetRuntimeCreate.createRuntimeException", e.getMessage()), (Exception)e);
        } 
        if (workingCopy == null) {
          handleError(ResourceHandler.getString(
                "TargetRuntimeCreate.createdWorkingCopy", this.targetId));
        } else {
          ...
          try {
            ...
            if (msg.indexOf(badMsg) >= 0) {
              log(
                  ResourceHandler.getString("TargetRuntimeCreate.badMessage", badMsg));
            } else {
  ...
  protected IRuntimeType getServerRuntimeType(String typeId) {
    ...
    if (runtimeType == null) {
      handleError(
          ResourceHandler.getString("TargetRuntimeCreate.getRuntimeTypeId", typeId));
    } else {
  ...
  protected void validateAttributes(IProgressMonitor monitor) throws BuildException {
    ...
    if (this.runtimeTypeId == null) {
      String msg = ResourceHandler.getString(
          "TargetRuntimeCreate.usingDefaultRuntimeTypeId", 
          WASv60Type);
      log(msg);
      this.runtimeTypeId = WASv60Type;
    } 
    if (this.runtimeName == null) {
      String msg = ResourceHandler.getString("TargetRuntimeCreate.usingDefaultTargetId", 
          this.targetId);
      log(msg);
