...
public class ProjectSetBuild extends FailOnErrorTask {
    ...
    try {
      ...
      if (projects == null || projects.size() == 0) {
        ...
        if (this.missingProjects) {
          ...
          msg = ResourceHandler.getString("Common.noProjectReferences", 
              this.projectSetFileName);
        } 
      ...
      log(
          ResourceHandler.getString("ProjectSetBuild.projectsToBuild", Integer.toString(projects.size())));
      String projectNames = "";
    ...
    for (int i = 0; i < projects.size(); i++) {
      ...
      if (this.useBuildXML) {
        ...
        if (workspaceProject == null) {
          log(
              ResourceHandler.getString("Common.message", ResourceHandler.getString("Common.projectNameNull", projName)));
          handleError(ResourceHandler.getString("Common.projectNameNull", projName));
          return "FAILED";
        } 
        if (!workspaceProject.exists()) {
          log(
              ResourceHandler.getString("Common.message", ResourceHandler.getString("Common.projectNameNotInWorkspace", projName)));
          handleError(
        ...
        if (!workspaceProject.isOpen()) {
          log(
              ResourceHandler.getString("Common.message", ResourceHandler.getString("Common.projectNotOpen", projName)));
        ...
        if (!buildFile.exists() || !buildFile.canRead()) {
          log(ResourceHandler.getString(
                "Common.message", 
                ResourceHandler.getString("ProjectSetBuild.missingProject", (Object[])new String[] { projName, buildpath })));
          handleError(
              ResourceHandler.getString("ProjectSetBuild.missingProject", (Object[])new String[] { projName, buildpath }));
          return "FAILED";
        } 
  ...
  private String processLine(String line) {
    ...
    if (workspaceProject != null && !workspaceProject.exists()) {
      log(
          ResourceHandler.getString("ProjectSetBuild.skippingMissingProject", projectName));
  ...
  protected void validateAttributes(IProgressMonitor monitor) throws BuildException {
    if (AntTrace.EXTRAS_TRACE_ENABLED) {
      ...
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
          "Value of attribute \"projectSetFileName\":" + this.projectSetFileName);
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
          "Value of attribute \"propertyBuildProjectNames\":" + this.propertyBuiltProjectNames);
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
          "Value of attribute \"useBuildXML\":" + this.useBuildXML);
      AntBundleActivator.getDebugTrace().trace("/debug/antextras", 
          "Value of attribute \"buildFileName\":" + this.buildFileName);
      AntBundleActivator.getDebugTrace().traceExit("/debug/antextras", 
          "Value of attribute \"buildTarget\":" + this.buildTarget);
    } 
    ...
    if (!this.projectSetFileName.substring(this.projectSetFileName.length() - 4, this.projectSetFileName.length()).equalsIgnoreCase(".psf"))
      handleError(
          NLS.bind(NLSMessageConstants.IMPORT_PROJECT_INVALID_PSF_EXTENSION, this.projectSetFileName)); 
    File file = new File(this.projectSetFileName);
    if (!file.isFile()) {
      log(
          ResourceHandler.getString("Common.message", ResourceHandler.getString("Common.projectSetFileIsNotAFile", this.projectSetFileName)));
      handleError(
    ...
    if (!file.exists()) {
      log(ResourceHandler.getString("Common.message", ResourceHandler.getString(
              "Common.projectSetFileDoesNotExist", this.projectSetFileName)));
      handleError(
          ResourceHandler.getString("Common.projectSetFileDoesNotExist", this.projectSetFileName));
    } 
    if (!file.canRead()) {
      log(ResourceHandler.getString("Common.message", ResourceHandler.getString(
              "Common.projectSetFileNonReadable", this.projectSetFileName)));
      handleError(
