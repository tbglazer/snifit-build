...
public class AntConsoleProgressMonitor implements IProgressMonitor {
	...
	public void subTask(String name) {
		if (!this.quiet && !this.summary && name != null && name.length() > 0 && !name.equalsIgnoreCase("Updating.")
	...
	public void displayMsg(String msg) {
		if (!this.quiet)
			this.task.log(ResourceHandler.getString("AntConsoleProgressMonitor.taskMessage",
					(Object[]) new String[] { this.taskname, msg }));
	}

	public void displayMsg(String msg, int level) {
		if (level == 0 || level == 1) {
			this.task.log(ResourceHandler.getString("AntConsoleProgressMonitor.taskMessage",
					(Object[]) new String[] { this.taskname, msg }), level);
		} else if (!this.quiet) {
			this.task.log(ResourceHandler.getString("AntConsoleProgressMonitor.taskMessage",
					(Object[]) new String[] { this.taskname, msg }), level);
		}
