			...
			if (!this.useEclipsePrefs) {
				...
				if (this.preferenceScopes == null) {
					if (AntTrace.EXTRAS_TRACE_ENABLED)
						AntBundleActivator.getDebugTrace().trace("/debug/antextras",
								ResourceHandler.getString("WorkspacePreferenceGet.defaultPreferencesScope", "default"));
				...
				if (preferenceValue != null) {
					if (this.propertyName != null) {
						...
						if (AntTrace.EXTRAS_TRACE_ENABLED)
							AntBundleActivator.getDebugTrace().trace("/debug/antextras",
									ResourceHandler.getString("WorkspacePreferenceGet.displayPreferenceValue",
											(Object[]) new String[] { this.preferenceName, preferenceValue,
													this.propertyName }));
					}
				} else if (AntTrace.EXTRAS_TRACE_ENABLED) {
					AntBundleActivator.getDebugTrace().trace("/debug/antextras", ResourceHandler
							.getString("WorkspacePreferenceGet.preferenceNotInWorkspace", this.preferenceName));
				}
			}
		} finally {
			monitor.done();
