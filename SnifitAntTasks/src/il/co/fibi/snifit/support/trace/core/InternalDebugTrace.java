...
public class InternalDebugTrace implements DebugTrace {
	...
	public void traceObject(String option, Object attribute) {
		...
		if (attribute != null) {
			...
			} else if (attribute instanceof Collection icollection) {
				message = InternalTrace.convertToString("Attribute trace", icollection);
			} else if (attribute instanceof Map imap) {
				message = InternalTrace.convertToString("Attribute trace", imap);
			} else {
